...
Previously a merchant account had a single "active" environment and the API
rejected requests sent with a key whose prefix didn't match. That gate is
gone. Each request is routed by the prefix on the key you send
(`cp_*_test_*` → test, `cp_*_live_*` → live), so you can run a staging
...
[Authentication](./authentication#test-and-live-keys-are-independent--use-them-side-by-side)
page for the full picture.

### Disabling the API in your dashboard preserves your keys

If a merchant disables the developer API in the dashboard, requests with
existing keys start returning `401 Unauthorized` (as expected), **but the
