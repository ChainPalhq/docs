...
<Note>
**About `reference` vs `clientReference` in `data`**: Both fields carry the
same value — the `clientReference` you supplied when initializing the payment
(or the one ChainPal generated for you if you didn't pass one). `reference`
is kept as a legacy alias; new integrations should read `clientReference`.
</Note>
